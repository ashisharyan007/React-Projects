  ...
  return (
    <section id='skills' className='py-24 pb-24 px-[12vw] md:px-[7vw] lg:px-[20vw] font-sans bg-skills-gradient'>
      ...
      <div className='text-center mb-8'>
        ...
        <p className='text-gray-400 mt-4 text-lg font-semibold'>An overview of the technologies I’ve mastered through practical experience and project work.</p>
      </div>
      ...
      <div className='flex flex-wrap gap-1 lg:gap-5 py-10 justify-between'>
        {SkillsInfo.map((category) => (
          <div key={category.title} className='bg-gray-900 backdrop-blur-md px-6 sm:px-10 py-8 sm:py-6 mb-10 w-full sm:w-[48%] rounded-2xl border border-white shadow-[0_0_20px_1px_rgba(130,69,236,0.3)]'>
            <h3 className='text-2xl sm:text-3xl font-semibold text-gray-400 text-center mb-4'>
              {category.title}
            </h3>

            {/* Skill Item   3 on larger screens*/}
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-4  '>
              {category.skills.map((skill) => (
                <div key={skill.name} className='flex items-center justify-center space-x-2 bg-transparent border-2 border-gray-700 rounded-3xl py-2 px-2 sm:py-2 sm:px-2 text-center'>
                  ...
                  <span className='text-xs sm:text-sm text-gray-300'>{skill.name}
                  </span>
                </div>
              ))}
            </div>
